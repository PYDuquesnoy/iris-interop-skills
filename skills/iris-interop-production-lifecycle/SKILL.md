---
name: iris-interop-production-lifecycle
description: Use when working with the IRIS Interoperability production class itself — adding/removing components, settings propagation, default site settings, start/stop/update, deployment between environments. Covers production XML structure and the lifecycle commands. Triggers: producción, production class, arrancar producción, parar producción, default settings, deploy, exportar producción.
---
...
Components are added by editing the XML directly or, more usually, via the Management Portal (or via iris-dev MCP).
...
3. After a deploy: smoke test by sending one canonical message through and using `iris-interop-message-search-debug` to confirm Visual Trace looks correct end-to-end.
...
- `iris-interop-business-services` — adding a BS to the production
- `iris-interop-business-operations` — adding a BO to the production
- `iris-interop-bpl` — adding a BP / Message Router
- `iris-interop-message-search-debug` — verifying live behaviour after a change; purge task lives in the production
- `iris-interop-alerting` — alert circuit baseline (alert router + sink BO + monitor service)
- `iris-interop-hl7-schemas` — schema export discipline (NOT auto-exported)
- `iris-interop-security` — credentials, instance keys, namespace-level security boundaries
