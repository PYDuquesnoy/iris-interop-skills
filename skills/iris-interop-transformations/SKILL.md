---
name: iris-interop-transformations
description: Use when writing or modifying data transformations — DTL (Data Transformation Language) for HL7/XML/object messages, subtransforms, HL7-specific transforms, and XSLT for CDA documents. Covers Create=New/Copy/Existing, foreach/if/switch, lookup functions, testing, and when to drop to ObjectScript code. Triggers: transformar, DTL, transformación, mapear, convertir mensaje, ADT a, v2.5 a v2.3.
---
...
`Lookup("TableName", source.field, "default-or-empty")` translates codes via a lookup table. See `iris-interop-lookup-tables`. Always specify the third parameter (default value or behaviour on miss) — the default-on-miss is silent and surprising otherwise.
...
- FHIR resource transformations — different patterns (`HS.FHIR.DTL.*`); see `iris-interop-fhir`.
- Procedural orchestration (multi-step workflows with side effects) — that's a BPL job, see `iris-interop-bpl`.

## See also

- `iris-interop-messages` — define the source and target message classes first; XML projection settings for output shape
- `iris-interop-lookup-tables` — Lookup() function source data
- `iris-interop-hl7-schemas` — when source/target HL7 needs custom Z-segments; escape special characters when building HL7 strings by hand
- `iris-interop-bpl` — when "transformation" is really orchestration
- `iris-interop-fhir` — FHIR DTL patterns (`HS.FHIR.DTL.*`)
- `iris-interop-business-operations` — lab device integration DT both directions; port pair documentation
