...
If the conversation is purely about wiring components into a production (production XML edits, settings), this skill doesn't apply — see `iris-interop-production-lifecycle`.
...
For runner mechanics — the `^UnitTestRoot` directory requirement, `DebugRunTestCase` qualifier syntax (boolean flags only), the MCP-friendly SqlProc wrapper that returns `passed=N failed=M`, how to read `^UnitTest.Result`, and the `Try / Catch + Quit` pitfall — see **`iris-interop-unit-tests`**. That skill is the framework toolbox; this one is the workflow.

After running, **always print the portal URL** as the last line of test output so the user can drill into per-assert detail. URL pattern and the navigation chain are also in `iris-interop-unit-tests`.

## Error-handling idiom inside test helpers

When test setup / teardown / fixture-builder code drops to ObjectScript, use the standard try/catch idiom — same pattern used in production code so failures surface uniformly. Pattern (and the `Quit` inside `Try` pitfall) is documented once in `iris-interop-unit-tests`.
...
See `iris-interop-business-operations` and `iris-interop-bpl` for the runtime side of the same rule.
...
- `iris-interop-messages` — design the contract before the tests
- `iris-interop-transformations` — DTL implementation reference (after the test exists)
- `iris-interop-business-operations` — keep BO methods thin so they're testable
- `iris-interop-bpl` — BPL Business Processes (test via Testing Service)
- `iris-interop-message-search-debug` — for inspecting Visual Trace after a Testing Service dispatch
- `iris-interop-unit-tests` — runner mechanics (`Run()`, `DebugRunTestCase`, SqlProc wrapper, qualifier syntax), `^UnitTest.Result` global, the `%UnitTest.Portal` URL, where to store tests so they survive. **This skill is the workflow; that one is the toolbox.**
