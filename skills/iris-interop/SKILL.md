...
In IRIS Interoperability, **messages are the foundational building block**. They are the parameters of Business Processes and Business Operations, and the response types of Business Services. Design and create messages **before** authoring BS/BP/BO. If the user asks to "build a service" without a message class in mind, stop and design the message first using `iris-interop-messages`.
...
Detailed migration patterns, the deployment tool, and the multi-environment configuration strategy live in `iris-interop-production-lifecycle`.
...
|---|---|
| Designing the message class itself (HL7, persistent, SOAP) | `iris-interop-messages` |
| Building a Business Service (inbound: file/TCP/SOAP/REST/CSV) | `iris-interop-business-services` |
| Writing a DTL or transforming HL7/CDA/XML | `iris-interop-transformations` |
| Building a non-SOAP Business Operation (TCP, SQL, file, REST) | `iris-interop-business-operations` |
| Building a SOAP Business Operation (wizard, WSDL gotchas, %Persistent payloads, CDA) | `iris-interop-soap-bo` |
| Writing a BPL Business Process or routing rules | `iris-interop-bpl` |
| Production class structure, start/stop, settings, deployment, migration | `iris-interop-production-lifecycle` |
| Custom HL7 schemas, Z-segments, schema editor | `iris-interop-hl7-schemas` |
| Lookup tables (creating, loading, using in DTL) | `iris-interop-lookup-tables` |
| Searching messages, Visual Trace, Event Log, debugging, testing live components, SOAP tracing, purge | `iris-interop-message-search-debug` |
| **FHIR work** — Façade vs Repository, OAuth2 PKCE, FHIR R4 Bundles, FHIR SQL Builder | `iris-interop-fhir` |
| **Securing endpoints** — SAML 2.0 / 1.1, OAuth 2.0 server + LDAP, SSL/TLS chain, internal account hygiene | `iris-interop-security` |
...
1. **Messages** (`iris-interop-messages`)
2. **Custom schemas** if needed (`iris-interop-hl7-schemas`)
3. **TDD workflow** (`iris-interop-tdd`) — every component below has its test written BEFORE the implementation. Non-negotiable.
4. **Business Service** (`iris-interop-business-services`) — entry point; testable only from outside IRIS
5. **Transformations** (`iris-interop-transformations`) — DTL test → DTL impl
6. **Business Process / routing rules** (`iris-interop-bpl`) — rule test → rule impl; BPL via Testing Service
7. **Business Operation** — `iris-interop-business-operations` (generic) or `iris-interop-soap-bo` (SOAP) — BO method test → BO impl
8. **Alert circuit** (`iris-interop-alerting`) — wire `Ens.Alert` router + sink BO + dedup function set; audit per-BO alert settings
9. **Production wiring + settings** (`iris-interop-production-lifecycle`) — add `TestingEnabled="true"` for dev productions; choose deployment tool early
10. **Security** (`iris-interop-security`) — only after components exist; SAML/OAuth/SSL added where the endpoints actually call out
11. **Test, search, debug** (`iris-interop-message-search-debug`) — Visual Trace + Event Log for verifying end-to-end runs; purge task added

For FHIR-specific work, replace steps 4–7 with the `iris-interop-fhir` decision tree (Façade vs Repository, OAuth2 PKCE setup, FHIR R4 Bundle shape).

## MCP dependency

The skills assume the `iris-dev` MCP server is enabled — it provides the actions for compiling classes, inspecting productions, listing messages. If MCP tools are not available in the current session, tell the user explicitly and ask them to enable iris-dev (or perform the action manually in the IRIS Management Portal).
