---
name: iris-interop-fhir
description: Use when building FHIR endpoints on IRIS for Health — Façade vs Repository decision, OAuth 2.0 + PKCE for mobile clients, privacy-preserving identity flow on public servers, FHIR R4 Bundle shapes, FHIR SQL Builder for SQL projection, deployment SOW out-of-scope checklist. Triggers: FHIR, fachada, façade, repository, FHIR R4, OAuth PKCE, Bundle, Patient, Observation, SMART-on-FHIR, FHIR SQL Builder.
---
...
Server-side IRIS OAuth configuration patterns are in `iris-interop-security`.
...
These map to standard Interop alerting (`iris-interop-alerting`) plus a couple of FHIR-specific dashboards.
...
- DICOM-to-FHIR conversion (combine this skill with `iris-interop-dicom`, which is currently a stub).

## See also

- `iris-interop-security` — OAuth 2.0 server setup, SSL/TLS chain, SAML when partner systems require it.
- `iris-interop-business-services` — REST inbound BS as the FHIR endpoint entry point.
- `iris-interop-transformations` — DTL for HL7v2 → FHIR resource shape (`HS.FHIR.DTL.*` reference).
- `iris-interop-production-lifecycle` — namespace strategy (FHIR typically lives in its own namespace).
- `iris-interop-alerting` — wiring the four mandatory FHIR-Façade monitoring concerns.
