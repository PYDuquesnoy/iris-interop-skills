---
name: iris-interop-hl7-schemas
description: Use when working with custom HL7 v2.x schemas — Z-segments, custom message structures, schema editor, schema categories, importing custom schema definitions. Triggers: schema HL7, esquema, Z-segment, ADT custom, HL7 personalizado.
---
...
7. **Assign to the BS** (see `iris-interop-business-services`):
...
Helper FunctionSet pattern (`${CLAUDE_PLUGIN_ROOT}/Mejores_Practicas/examples/ch02_hl7v2/hl7v2-escape-functionset.cls`) is portable — copy into your namespace, expose `FormataTextPerHL7v2(text)` and the inverse `DesformataTextDesdeHL7v2(text)`, call from BPs and DTLs.
...
- `iris-interop-messages` — the message class that wraps schema-validated content
- `iris-interop-transformations` — DTL DocTypes draw from these schemas
- `iris-interop-business-services` — BS schema-version setting
- `iris-interop-production-lifecycle` — deployment bundle and what travels with it (and what doesn't)
