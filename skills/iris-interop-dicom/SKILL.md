---
name: iris-interop-dicom
description: Use when the user mentions DICOM, modality worklists (MWL), C-STORE, C-FIND, C-MOVE, PACS, STOW-RS, DICOMweb, AE Title, or any EnsLib.DICOM.* class. Triggers ES: DICOM, modalidad, modalidades, worklist, PACS, imagen médica. Triggers EN: DICOM, modality, worklist, MWL, PACS, C-STORE, C-FIND, C-MOVE, STOW-RS, DICOMweb, AE Title, association context.
---
...
sample). See `iris-interop-business-operations` for the SQL outbound shape and
...
> Language Server (see `iris-interop-business-operations` friction #63). New
...
application + dispatch class). See `iris-interop-business-services` for the
...
2. A standard router (`iris-interop-bpl`) sends the custom message to a DTL
...
`iris-interop-messages` (custom message design), `iris-interop-transformations`
(DTL), `iris-interop-bpl` (router), `iris-interop-fhir` (FHIR resource shape).
...
- **Pixel data purge** — DICOM messages embed large binary streams. The
  ...
  files. Plan a file-stream-aware purge task (`iris-interop-production-lifecycle`).
...
- `iris-interop-production-lifecycle` — wiring DICOM components, purge tasks
  for streamed pixel data, `OnStart` patterns.
- `iris-interop-business-services` — when the gateway side is non-DICOM
  (HTTP, file, REST) and for `/csp/<app>/` web-app registration of STOW-RS.
- `iris-interop-business-operations` — SQL outbound for MWL source data, JDBC
  / External Language Server setup.
- `iris-interop-messages` — custom `%Persistent` `Ens.Request` classes
  carrying tags extracted from DICOM into the canonical pipeline.
- `iris-interop-transformations` — DTL on the canonical custom message (NOT
  on `EnsLib.DICOM.Document` directly).
- `iris-interop-bpl` — routers in the bridge half of a DICOM↔HL7/FHIR gateway.
- `iris-interop-fhir` — `ImagingStudy` / `DiagnosticReport` for DICOM→FHIR.
- `iris-interop-security` — TLS configuration for DIMSE mutual auth.
