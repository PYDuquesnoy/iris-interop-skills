---
name: iris-interop-business-operations
description: Use when building generic (non-SOAP) IRIS Interoperability Business Operations — outbound endpoints sending to TCP/MLLP, files, FTP, HTTP, SQL/JDBC, REST. Covers adapter choice, custom BO methods, MessageMap dispatch. For SOAP-specific BOs (wizard, %SerialObject vs %Persistent, CDA), use iris-interop-soap-bo. Triggers: enviar mensajes, business operation, outbound, BO, SQL outbound, REST outbound, TCP outbound, file outbound.
---
...
If the destination is a SOAP web service, **stop and use `iris-interop-soap-bo`** instead. It covers the SOAP Wizard, %SerialObject vs %Persistent payload decision, the delete trigger, and the recursive-CDA pattern. This skill stays focused on non-SOAP BOs.
...
Set BO timeouts last, after the calling BP's timeout is fixed. See `iris-interop-bpl` for the BP-side view.
...
This **Completes** the message regardless of reply code; the BP receives the response and decides what to do. See `iris-interop-alerting` for the full decision matrix.
...
4. Use `iris-interop-message-search-debug` Visual Trace — confirm the BO received, attempted, and got an ACK/response from the destination.
...
See `iris-interop-alerting` for the full picture; this is the per-BO subset.
...
- **SOAP** outbound → `iris-interop-soap-bo` (wizard + storage decisions for the generated classes).
- DICOM C-STORE outbound — see `iris-interop-dicom` (stub).
...
- `iris-interop-soap-bo` — SOAP-specific BO (wizard, %Persistent payloads, CDA, WSDL gotchas)
- `iris-interop-messages` — design the request type the BO consumes
- `iris-interop-transformations` — DTL upstream produces the message shape the BO expects; lab device DT both directions
- `iris-interop-production-lifecycle` — wiring the BO into the production
- `iris-interop-unit-tests` — refactor BO methods to be testable
- `iris-interop-message-search-debug` — confirm outbound delivery
- `iris-interop-alerting` — per-BO settings checklist (Send Alert on Error, QueueWaitAlert, Failure Timeout, ReplyCodeActions matrix)
- `iris-interop-bpl` — timeout precedence (BO < BP)
